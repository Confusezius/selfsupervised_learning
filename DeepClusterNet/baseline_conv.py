...
class BaseConv_Scaffold(nn.Module):
    def __init__(self, num_classes, funnel_type='fullt-convolutional'):
...
class AlexNet(nn.Module):
    def __init__(self, num_classes, input_size, use_sobel=False, use_toplayer=True, use_batchnorm=False, config=None):
    ...
    def __GiveMLP(self):
        ...
        self.MLP_wo_top = nn.Sequential(nn.Linear(self.extend_length, 4096),
        ...
        self.top_layer = nn.Linear(4096, self.pars['num_classes']) if self.pars['use_toplayer'] else None
    ...
    def forward(self, x):
        if self.pars['use_sobel']: x = torch.cat([x,self.sobel(x)], dim=1)

        x = self.features(x)
        x = x.view(x.size(0), -1)

        x = self.MLP_wo_top(x)
        if self.top_layer is not None: x = self.top_layer(x)
